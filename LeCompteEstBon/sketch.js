...
let nbTiles = 6;
let large;
let YTilesDeBase = 250;
let SizeTilesDeBase = 150;
...
function windowResized() {
  ...
  c = tiles[0].pos.x - tiles[0].size/2;
  phaseX = [c, c+100, c+150, c+300];
  b = this.MyW /2 - tiles[6].size -25;
  ...
  if(videoSlider != undefined){ videoSlider.position(width - 160, height -25);}
    ...
    for(var i =0;i<mesBoutons.length ;i++){
      if(mesBoutons[i].touche()){
        if(mesBoutons[i].callback != null){
          if(mesBoutons[i].touche()){this[mesBoutons[i].callback]();}
        }else{
...
function setupGame(){
  for (var i = 0; i < nbTiles; i++) {
    tiles.push(new Tile(0,200 + 225 * i, YTilesDeBase, SizeTilesDeBase));
    large = i * 225;
  }
  large += 400;
  tiles.push(new Tile(0,large / 2, 110, 200));
  tiles[tiles.length - 1].sansCadre = true;
  ligneDeCalcul.push(new LigneDeCalcul(0));
  time = millis();
  times = new Tile("02:00",width-190,95,250);
}
function setupBoutons(){
  mesBoutons.push(new Tile("NEW",60,30,50));
  mesBoutons[0].callback = "rejouerEnRandom";
  mesBoutons.push(new Tile("RAZ",60,80,50));
  mesBoutons[1].callback = "RAZ";
  mesBoutons.push(new Tile("<<<",300,140,70));
  mesBoutons[2].callback = "Backspace";
  mesBoutons.push(new Tile("+",400,80,70));
  mesBoutons.push(new Tile("-",400,140,70));
  mesBoutons.push(new Tile("/",500,80,70));
  mesBoutons.push(new Tile("X",500,140,70));
